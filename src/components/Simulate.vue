<script setup></script>

<template>
  <div
    class="flex flex-wrap justify-between bg-white p-3 rounded-xl shadow-sm shadow-slate-300"
  >
    <div>
      <p class="font-bold text-sm">Try the full loop quickly</p>
      <p class="text-slate-500 text-sm tracking-tight">
        Simulates realistic vendor submissions (including a decline, a partial
        quote, a late submission and a clarification) so you can jump straight
        to comparison, award and costing.
      </p>
    </div>
    <div
      class="text-white bg-[#4f4dc2] flex justify-center items-center px-3 text-sm rounded-lg font-bold cursor-pointer"
    >
      Simulate vendor submissions
    </div>
  </div>
</template>
